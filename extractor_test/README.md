...
- **`_text_number_tokens(text) -> tuple`**（`@lru_cache`）
  原文中的金额 token 数值列表（含 `万元/亿元` 等单位换算 + **汉字金额归一化**），按原文缓存供 `_amount_token_match` 复用，避免重复全文扫描与逐 token 解析。全文只扫描一次。

- **`_amount_token_match(target, text) -> bool`**
  目标金额是否与原文某个金额 token **数值相等**（含 `万元/亿元` 换算、**汉字金额归一化**：`80000` 可匹配 `8万元`，`2000` 可匹配 `二千`/`贰仟`）。

- **`_cn_amount(s) -> int | None`**
  汉字金额串 → 数值（如 `二千`→`2000`、`贰仟元`→`2000`、`两万`→`20000`、`一万零三百`→`10300`、`十万`→`100000`）。仅当串含数量级单位（十/百/千/万/亿）或带 `元/人民币/整` 后缀时才判定为金额，避免误识别日期等；解析失败返回 `None`。
...
- **`_parse_amount(s)`**
  金额串 → 数值（元）：**汉字金额（`二千`/`贰仟`/`两万`/`一万零三百`）先归一化为数字**；阿拉伯数字支持 `万元 ×10000`、`亿元 ×1亿`、去千分位/单位/全角数字；**带小数返回 float 全精度，整数金额返回 int**；无法解析返回 `None`。

- **`normalize_value(value, field)`**
  字段值归一化：`NUMBER_FIELDS` 统一转数值(元)，复用 `_parse_amount`；递归处理 dict/list；非数字字段原样返回。

- **数字字段溯源先归一化再比对**
  数字字段（`NUMBER_FIELDS`）溯源判定前，先把提取值与原文金额候选各自经 `_parse_amount` 归一化为数值(元)，再做**严格数值相等**比较。原文金额为汉字时同样归一化为数字（`2000` 可匹配 `二千元`/`贰仟元`）。含 `万元/亿元` 换算（`80000` 可匹配 `8万元`，反之 `8` 不能匹配 `8万元`）。无字符串归一化兜底路径。

### 3.4 数字字段溯源兜底重生成

- **`_source_amount_values(text, window=100) -> tuple`**（`@lru_cache`）
  从原文抽取金额候选（**含来源片段**）：各 `CLAIM_TRIGGERS` 触发词（诉讼请求区 / 标的额 / 赔偿金 / 裁判区，与各 skill 字段触发词对齐）后 `window` 字符内匹配带单位金额（`AMOUNT_TOKEN_RE`）及**汉字金额**（`_CN_AMOUNT_RE`）并解析为数值。返回 `((数值, 原文片段), ...)`，按数值去重；片段供重生成提示让模型**按多段原文复核**。**按原文缓存**（`provenance_fails` / `_regen_hint` / `restore_amount_precision` 反复调用，全文只扫描一次）；返回 `tuple`（不可变，防缓存被改）。
...
- **`provenance_fails(cand, text) -> list[str]`**
  ...
  - **数字字段**：数值与原文某金额 token **精确相等**（含 `万元/亿元` 换算、**汉字金额归一化**：`2000` 可匹配 `二千元`/`贰仟元`）→ 通过；字段 ∈ `SUMMED_FIELDS` 且值 = 原文金额候选的**精确**子集和 → 通过；其余判不通过（触发重生成）。
...
- 数字字段溯源**先归一化再比对**：提取值与原文金额候选各自经 `_parse_amount` 归一化为数值(元)后做严格相等比较，仅数值相等通过。原文汉字金额同样归一化（`2000` 可匹配 `二千元`/`贰仟元`），含 `万元/亿元` 换算（`80000` 可匹配 `8万元`）。
...
| `_text_number_tokens` | 原文 | `_amount_token_match` 每次重复全文扫描+逐 token 解析（含汉字金额） | 1 次 |
