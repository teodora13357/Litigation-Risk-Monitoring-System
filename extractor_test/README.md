...
| `AMOUNT_TOKEN_RE` | 带金额标记的阿拉伯金额正则 | 须带货币符号/单位，排除年份/案号/日期 |
...
- **`_amount_token_match(target, text) -> bool`**
  目标金额是否与原文某个金额 token **数值相等**（含 `万元/亿元` 换算、**汉字金额归一化与角/分**：`80000` 可匹配 `8万元`，`2000` 可匹配 `二千元`/`贰仟元`，`1234.56` 可匹配 `壹仟贰佰叁拾肆元伍角陆分`）。

- **`_cn_amount(s) -> int | float | None`**
  汉字金额串 → 数值（如 `二千`→`2000`、`贰仟元`→`2000`、`两万`→`20000`、`一万零三百`→`10300`、`十万`→`100000`、`壹仟贰佰叁拾肆元伍角陆分`→`1234.56`、`伍角`→`0.5`、`三分`→`0.03`）。仅当串含数量级单位（十/百/千/万/亿）或带 `元/人民币/整/角/分` 标记时才判定为金额，避免误识别日期等；解析失败返回 `None`。
...
- **`_parse_amount(s)`**
  金额串 → 数值（元）：**汉字金额（`二千`/`贰仟`/`两万`/`一万零三百`/`壹仟贰佰叁拾肆元伍角陆分`）先归一化为数字**；阿拉伯数字支持 `万元 ×10000`、`亿元 ×1亿`、去千分位/单位/全角数字；**带小数返回 float 全精度，整数金额返回 int**；无法解析返回 `None`。
...
- **`provenance_fails(cand, text) -> list[str]`**
  ...
  - **数字字段**：数值与原文某金额 token **精确相等**（含 `万元/亿元` 换算、**汉字金额归一化**：`2000` 可匹配 `二千元`/`贰仟元`）→ 通过；字段 ∈ `SUMMED_FIELDS` 且值 = **全文金额 token（`_text_number_tokens`，保留重复金额，不受触发词窗口限制）** 的**精确**子集和 → 通过；其余判不通过（触发重生成）。
...
- **`restore_amount_precision(value, text, field)`**
  ...
  1. 直接命中：与全文金额 token（保留重复金额）中某金额在 `AMOUNT_TOL` 内 → 用该原文金额；
...
- 溯源侧**只认带金额标记的数字**：阿拉伯金额须带货币符号/单位，汉字金额须带 `元/人民币/整/角/分`，排除年份、案号、日期、法条、期限等非金额数字；汉字金额支持角/分（`壹仟贰佰叁拾肆元伍角陆分` → `1234.56`）。
